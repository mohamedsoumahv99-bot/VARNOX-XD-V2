...
function readState() {
    try {
        if (!fs.existsSync(PMBLOCKER_PATH)) return { activé: false, message: '⚠️ Direct messages are blocked!\nYou cannot DM this bot. Please contact the owner in group chats only.' };
        ...
        return { activé: false, message: '⚠️ Direct messages are blocked!\nYou cannot DM this bot. Please contact the owner in group chats only.' };
    }
}

function writeState(activé, message) {
    try {
...
async function pmblockerCommand(sock, chatId, message, args) {
    ...
    if (sub.toLowerCase() === 'status') {
        await sock.sendMessage(chatId, { text: `PM Blocker is currently *${state.activé ? 'ON' : 'OFF'}*\nMessage: ${state.message}` }, { quoted: message });
        return;
    }
    ...
    if (sub.toLowerCase() === 'setmsg') {
        ...
        writeState(state.activé, newMsg);
