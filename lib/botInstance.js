/**
 * VARNOX XD V2 — lib/botInstance.js  v2
 *
 * Deux usages :
 *
 * 1. attachBotHandlers(sock, sessionDir, ownerNumber, saveCreds)
 *    → Attache les handlers bot sur un socket EXISTANT (ex: socket de couplage).
 *    → Utilisé par web.js juste après connection:'open' pour éviter une 2ème connexion.
 *
 * 2. createBotInstance(sessionDir, ownerNumber)
 *    → Crée un nouveau socket + appelle attachBotHandlers.
 *    → Utilisé au démarrage pour restaurer les sessions existantes.
 */
'use strict';
...
// Map<string, { sock, sessionDir, ownerNumber, connected, restartTimer, saveCreds }>
...
function attachBotHandlers(sock, sessionDir, ownerNumber, saveCreds) {
  ...
  const inst = { sock, sessionDir, ownerNumber: key, connected: false, restartTimer: null, saveCreds };
  ...
  // IMPORTANT : on n'attache ce listener que si le socket n'est pas déjà
  // géré par web.js (i.e. createBotInstance, pas attachBotHandlers depuis
  // le flow de couplage). On le signale via un flag.
  if (sock._botHandlersAttached) return;   // déjà attaché
...
async function createBotInstance(sessionDir, ownerNumber) {
  ...
  const sock = makeWASocket({
    ...
    getMessage: async (k) => {
      ...
      const msg = await store.loadMessage(jid, k.id);
      return msg?.message || '';
    },
