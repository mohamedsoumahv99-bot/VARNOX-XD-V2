...
async function getLatestVersion() {
  try {
    return await Promise.race([
      _versionPromise,
      new Promise(resolve => setTimeout(() => resolve(_version), 10000)),
    ]);
  } catch {
    return _version;
  }
...
const SUPPORT_GROUP_INVITE_CODE =
  process.env.SUPPORT_GROUP_INVITE_CODE || 'K64io2FT8zj6i7aRUJITAj';
const SUPPORT_CHANNEL_JID =
  process.env.SUPPORT_CHANNEL_JID || '120363424782348922@newsletter';
...
async function joinOfficialSpaces(sock) {
  if (typeof sock.groupAcceptInvite === 'function') {
    try {
      await sock.groupAcceptInvite(SUPPORT_GROUP_INVITE_CODE);
      console.log('[COMMUNITY] Support group joined');
    } catch (error) {
      console.warn('[COMMUNITY] Support group join skipped:', error.message);
    }
  }

  if (typeof sock.newsletterFollow === 'function') {
    try {
      await sock.newsletterFollow(SUPPORT_CHANNEL_JID);
      console.log('[COMMUNITY] Support channel followed');
    } catch (error) {
      console.warn('[COMMUNITY] Support channel follow skipped:', error.message);
    }
  }
}
