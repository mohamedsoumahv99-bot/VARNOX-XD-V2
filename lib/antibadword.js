...
async function handleAntiBadwordCommand(sock, chatId, message, match) {
    ...
    if (match === 'on') {
        ...
        if (existingConfig?.enabled) {
            return sock.sendMessage(chatId, { text: '⚠️ VARNOX antibadword est déjà activé.' }, { quoted: message });
        }
        await setAntiBadword(chatId, 'on', 'delete');
        return sock.sendMessage(chatId, { text: '✅ VARNOX antibadword activé. Action actuelle : suppression.' }, { quoted: message });
    }

    if (match === 'off') {
        const config = await getAntiBadword(chatId, 'on');
        if (!config?.enabled) {
            return sock.sendMessage(chatId, { text: '⚠️ VARNOX antibadword est déjà désactivé.' }, { quoted: message } );
        }
        await removeAntiBadword(chatId);
        return sock.sendMessage(chatId, { text: '🔕 VARNOX antibadword désactivé.' }, { quoted: message } );
    }

    if (match.startsWith('set')) {
        const action = match.split(' ')[1];
        if (!action || !['delete', 'kick', 'warn'].includes(action)) {
            return sock.sendMessage(chatId, { text: '❌ Action invalide. Choisis delete, kick ou warn.' }, { quoted: message } );
        }
        await setAntiBadword(chatId, 'on', action);
        return sock.sendMessage(chatId, { text: `✅ VARNOX antibadword configuré sur : ${action}.` }, { quoted: message } );
    }

    return sock.sendMessage(chatId, { text: '❌ Option invalide. Utilise .antibadword pour l’aide.' }, { quoted: message } );
}
